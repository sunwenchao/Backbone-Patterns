	...
	minify.prototype = minify.fn = {
		...
		compress: function() {
			...
			switch (this.type) {
                ...
                case 'yuicss':
                    command = 'java -jar -Xss2048k "' + __dirname + '/yuicompressor-2.4.7.jar" "' +
                        this.fileIn + '" -o "' + this.fileOut + '" --type css ' + this.options.join(' ');
                    break;
				...
				case 'gcc':
					...
					command = 'java -jar "' + __dirname + '/google_closure_compiler-r1810.jar" ' + fileInCommand.join(' ') + ' --js_output_file="' + this.fileOut + '" ' + this.options.join(' ');
					break;
				case 'uglifyjs':
